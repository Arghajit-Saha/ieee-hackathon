...
export async function POST(request: Request) {
    try {
        ...
        const { region, history, forceAssessment } = body;

        if (!region) {
            return NextResponse.json({ error: 'Region is required' }, { status: 400 });
        }

        const ragResponse = await fetch(`${RAG_API_URL}/query_dynamic`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ region, history: history || [], forceAssessment: !!forceAssessment })
        });

        if (!ragResponse.ok) {
            const errData = await ragResponse.json().catch(() => ({}));
            console.error('Dynamic RAG API error:', errData);
        ...
        let triageId = null;
        if (data.isComplete && data.assessment && data.assessment.summary) {
            ...
            try {
                ...
                const symptoms = [`Region: ${region}`, ...userMessages];
                ...
                const lng = body.location?.lng || (27.78 + (Math.random() - 0.5) * 0.05);
                const lat = body.location?.lat || (-26.15 + (Math.random() - 0.5) * 0.05);
                ...
                triageId = triageLog._id.toString();
        ...
        console.error('Dynamic Triage API Error:', error);
        return NextResponse.json(
            { error: 'Failed to connect to the RAG server.' },
            { status: 500 }
        );
    }
}

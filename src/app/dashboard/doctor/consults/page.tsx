...
function TeleconsultRoom() {
    ...
    const roomId = searchParams.get('room') || 'demo-room-123';

    const [hasJoined, setHasJoined] = useState(false);
    const [socket, setSocket] = useState<Socket | null>(null);
    const [userId] = useState(`doctor-${Math.floor(Math.random() * 10000)}`);
    ...
    return (
        <DashboardLayout>
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-black tracking-tight uppercase">Teleconsultation Room</h1>
                    <p className="font-mono-ui text-[10px] tracking-wider text-zinc-500 mt-1">SESSION ID: {roomId}</p>
                </div>
                <div className="flex items-center gap-2">
                    <span className="flex w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
                    <span className="font-mono-ui text-[10px] tracking-wider uppercase font-bold text-red-500">LIVE</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[70vh]">
                ...
                <div className="lg:col-span-2 bg-black border-2 border-black flex flex-col brutal-shadow overflow-hidden relative">
                    <div className="flex-1 relative bg-zinc-900 border-b-2 border-black flex items-center justify-center">
                        ...
                        {!remoteVideoRef.current?.srcObject && (
                            <p className="absolute uppercase font-bold text-zinc-600 font-mono-ui tracking-widest text-xs">Waiting for patient...</p>
                        )}
                ...
                <div className="bg-white border-2 border-black brutal-shadow flex flex-col">
            ...
            {!hasJoined && (
                <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6">
                    <div className="bg-white border-4 border-black p-8 md:p-12 brutal-shadow max-w-md w-full text-center">
                        ...
                        <h2 className="text-3xl font-black tracking-tight mb-4 uppercase">Patient Consult</h2>
                        <p className="text-zinc-500 text-sm mb-8 leading-relaxed">
                            The patient is waiting in the digital room. Click below to start the video consultation session.
                        </p>
                        <button
...
export default function Page() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-[#fafafa] flex items-center justify-center"><p className="font-mono-ui tracking-widest text-xs animate-pulse">LOADING TELECONSULT...</p></div>}>
            <TeleconsultRoom />
        </Suspense>
    );
}
